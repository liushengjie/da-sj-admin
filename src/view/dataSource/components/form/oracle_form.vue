<template>
	<div>
		<Modal :loading="true" :title="modalTitle" v-model="modalShow" :mask-closable="false" @on-cancel="closeNewDiag">
			<Form ref="dataForm" :model="dsParam" :labelWidth="90" :rules="ruleValidate">
				<FormItem label="源名称" prop="name">
					<Input placeholder="连接名称" v-model="dsParam.name"></Input>
				</FormItem>
				<FormItem label="源类型" prop="type">
					<Select v-model="dsParam.type">
						<i-option v-for="type in typeList" v-model="type.code">{{type.name}}</i-option>
					</Select>
				</FormItem>
				<FormItem label="IP地址" prop="ip">
					<Input placeholder="IP地址" v-model="dsParam.ip"></Input>
				</FormItem>
				<FormItem label="端口号" prop="port">
					<Input placeholder="端口号" v-model="dsParam.port"></Input>
				</FormItem>
				<FormItem label="数据库名称" prop="database">
					<Input placeholder="数据库名称" v-model="dsParam.database"></Input>
				</FormItem>
				<FormItem label="用户名" prop="username">
					<Input placeholder="用户名" v-model="dsParam.username"></Input>
				</FormItem>
				<FormItem label="密码" prop="pwd">
					<Input placeholder="密码" type="password" v-model="dsParam.pwd"></Input>
				</FormItem>
				<FormItem label="分布式事务">
					<RadioGroup v-model="dsParam.xa">
...
<script>
	...
	export default {
		...
		methods: {
			...
			connect(cb) {
				dbApi.testConn(this.dsParam).then(res => {
					if (res.success) {
						cb(true)
					} else {
						cb(false)

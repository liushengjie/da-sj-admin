<template>
    <div class="op-area">
        ...
        <Row class="col-row ele-scroll">
            ...
            <CellGroup>
                <Cell class="col-list" v-show="item.status==='1'||showHideCol" :disabled="item.status==='0'" v-for="(item, index) in $store.state.resource.resColList" :title="item.col" :label="item.alias">
        ...
        <columnFilter :previewData="previewData" @renderColList="renderColList" :dsType="curDsType" :colList="$store.state.resource.resColList" ref="colList"></columnFilter>

        ...
        <Modal v-model="filterModalShow" title="筛选器" width="900" @on-ok="saveFilter" @on-cancel="init">
            <div style="height: 200px;">
                <div class="split-type-list">
                    <CellGroup>
                        <Cell  v-for="filter in filterList" :selected="filter.selected" :data-id="filter.id" :data-name="filter.name" :title="filter.name" :name="filter.id" class="draggable1"></Cell>
                    </CellGroup>
                </div>
  ...
  methods: {
    ...
    showFilterModal () {
      ...
      this.$nextTick(() => {
        ...
        this.visNetwork.on('doubleClick', function (params) {
          console.log(params)
          this.body.data.nodes.remove({ id: params.nodes[0] })
          _this.curFilterId = ''
    ...
    dragEnd (filterId, filterName) {
      ...
      if (!this.curFilterList.includes(this.curFilterId)) {
        ...
        this.visNetwork.body.data.nodes.add({ id: filterId, label: filterName, level: 2 })
        this.visNetwork.body.data.edges.add([{ id: 'addEdge' + filterId, from: this.curColName, to: filterId }])
      }
    },
    saveFilter () {
